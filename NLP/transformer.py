...
# Activation = Enum('Activation', 'ReLU Swish Mish TAct mTAct')
...
_activ_func = {Activation.ReLU:nn.ReLU(inplace=True)}

def feed_forward(d_model:int, d_ff:int, ff_p:float=0., act=Activation.ReLU, double_drop:bool=True):
...
tfmer_lm_config = dict(ctx_len=512, n_layers=12, n_heads=12, d_model=768, d_head=64, d_inner=3072, resid_p=0.1, attn_p=0.1,
                         ff_p=0.1, embed_p=0.1, output_p=0., bias=True, scale=True, act=Activation.ReLU, double_drop=False,
...
#Argument values are wrong
model = Transformer(vocab_sz=VOCAB_SIZE, ctx_len=5, n_layers=2, n_heads=2, d_model=512, d_head=64, d_inner=3072).to(device)
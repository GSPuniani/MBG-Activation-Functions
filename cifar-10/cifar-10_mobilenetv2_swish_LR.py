...
epochs = 100
batch_size = 128
model = mobilenetv2(activation = 'swish')
LRVec = [0.0001, 0.01, 0.1, 1]

for j in range(1, 4):
    for i in range(len(LRVec)):
        learning_rate = LRVec[i]
        ...
        train_stats.to_csv(f'MobileNetv2_Swish_LR{LRVec[i]}_run{j}.csv')
